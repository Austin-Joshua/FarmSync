  ...
  private async request<T>(
    ...
    try {
      ...
      if (error.name === 'AbortError') {
        throw new Error(`Request timed out. Start the backend first: run "cd Backend && npm run dev" (backend: ${BACKEND_ORIGIN}).`);
      }
      ...
      if (error.name === 'TypeError' && (error.message.includes('fetch') || error.message.includes('Failed') || error.message.includes('NetworkError'))) {
        throw new Error(`Failed to connect to server. Start the backend first: run "cd Backend && npm run dev" (backend: ${BACKEND_ORIGIN}).`);
      }
